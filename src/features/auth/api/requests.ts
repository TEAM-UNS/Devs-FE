...
export async function signup(body: SignupRequest): Promise<MessageResponse> {
  return post<MessageResponse>('/signup', body)
}
...
export async function sendEmailCode(
  ...
  return post<MessageResponse>('/email/send', body)
}

/** 이메일 인증코드를 검증한다. (POST /email/verify) */
export async function verifyEmail(
  body: VerifyEmailRequest,
): Promise<MessageResponse> {
  return post<MessageResponse>('/email/verify', body)
}
