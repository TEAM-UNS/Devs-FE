...
/**
 * 전공 한 개. `id`가 회원가입의 `major_ids`이자 대시보드의 `major_id`가 된다.
...
/** 전공 목록을 조회한다. (GET /majors — 인증 카테고리와 달리 최상위 경로다) */
...
/**
 * 전공 목록 쿼리 정의.
 * 거의 바뀌지 않는 목록이라 `staleTime`을 길게 둬 화면을 오갈 때 재요청하지 않는다.
 */
